...
day10 = sum . map (score "") . parseInput
...
day10b = middle . sort . filter (>0) . map (score2 "") . parseInput

-- | Testing score2
-- >>> map (score2 "") . parseInput $ testInput
...
testInput :: String
testInput = unlines $ filter (not . null) $ lines [r|
[({(<(())[]>[[{[]{<()<>>
[(()[<>])]({[<{<<[]>>(
{([(<{}[<>[]}>{[]{[(<()>
(((({<>}<{<{<>}{[]{[]{}
