...
-- 
-- Both methods `<+>` and `<->` consider 6 different classes of overlap of the two regions:
-- 
--  A     B
-- ----- ------  (1)
-- 
...
instance Geometry Interval where
    x@(a,b) <+> y@(c,d)
        | b < c                   {- (1) -} = Set.fromList [x,y]
        | a < c && b > c && b < d {- (2) -} = Set.fromList [(a,c),(c,b),(b,d)]
        | a < c && b > d          {- (3) -} = Set.fromList [x]
        | a > c && b < d          {- (4) -} = Set.fromList [y]
        | a > c && a < d && b > d {- (5) -} = Set.fromList [(c,a),(a,d),(d,b)]
        | a > d                   {- (6) -} = Set.fromList [y,x]
        | otherwise = error "partial <+>"
    x@(a,b) <-> y@(c,d)
        | b < c                   {- (1) -} = Set.fromList [x]
        | a < c && b > c && b < d {- (2) -} = Set.fromList [(a,c)]
        | a < c && b > d          {- (3) -} = Set.fromList [(a,c),(d,b)]
        | a > c && b < d          {- (4) -} = Set.fromList []
        | a > c && a < d && b > d {- (5) -} = Set.fromList [(d,b)]
        | a > d                   {- (6) -} = Set.fromList [x]
        | otherwise = error "partial <->"
...
day22b = sum . map volume . Set.toList . foldl' toggles Set.empty . parseInput
    where
    toggles :: Set.Set Cuboid -> (Bool, Cuboid) -> Set.Set Cuboid -- TODO: Could have edge-cases
    toggles s c = Set.unions $ Set.map (`toggle` c) s

    toggle :: Cuboid -> (Bool, Cuboid) -> Set.Set Cuboid
    toggle s (b,c) = bool (s<->c) (s<+>c) b
