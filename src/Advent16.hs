...
import Data.Bits (testBit, Bits (zeroBits, clearBit, setBit))
import Control.Arrow (Arrow(first, (&&&)))
import Text.Megaparsec ( many, sepBy, parse, single, Parsec, ParseErrorBundle, parseMaybe, MonadParsec (takeP, token, takeWhile1P, takeWhileP), satisfy )
...
sumVersions (Packet (Header v _) i) = case i of
  Lit _  -> vNum
  Ops os -> vNum + sum (map sumVersions os)
  where
  vNum = fromIntegral v

-- | Examples
-- >>> day16 "8A004A801A8002F478"
-- 16
-- 
-- >>> day16 "620080001611562C8802118E34"
-- 12
-- 
-- >>> day16 "C0015000016115A2E0802F182340"
-- 23
-- 
...
operatorItems :: Parsec () [(Integer, Bool)] [Packet]
operatorItems = do
  b <- snd <$> token Just Set.empty
  if b
    then bitsAsNum 11 >>= subPackets
    else bitsAsNum 15 >>= lengthInBits

lengthInBits :: Int -> Parsec () [(Integer, Bool)] [Packet]
lengthInBits n = do
  bs <- takeP Nothing n
  pure $ either (error.show) id $ parse (many solve2) "" bs

subPackets :: Int -> Parsec () [(Integer, Bool)] [Packet]
subPackets n = replicateM n solve2

bitsAsNum :: Int -> Parsec () [(Integer, Bool)] Int
bitsAsNum n = boolsNum . map snd <$> takeP Nothing n

literalBits :: Parsec () [(Integer, Bool)] [Bool]
literalBits = do
  b <- snd <$> token Just Set.empty
  v <- takeP Nothing 4
  let v' = map snd v
  ...
  if b
    then (v'++) <$> literalBits
      ...
      -- takeWhileP Nothing (\t@(a,b) -> a == x)
      pure v'

header :: Parsec () [(Integer,Bool)] Header
header = do
...
boolsNum :: Bits a => [Bool] -> a
boolsNum = foldr setBool zeroBits . zip [0..] . reverse

setBool :: Bits a => (Int, Bool) -> a -> a
setBool (n, False) b = clearBit b n
setBool (n, True ) b = setBit b n
